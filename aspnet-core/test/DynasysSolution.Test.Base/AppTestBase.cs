...
using DynasysSolution.Test.Base.TestData;
...
namespace DynasysSolution.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="DynasysSolutionDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(DynasysSolutionDbContext context)
            {
        ...
        protected void UsingDbContext(Action<DynasysSolutionDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DynasysSolutionDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<DynasysSolutionDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<DynasysSolutionDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DynasysSolutionDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DynasysSolutionDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DynasysSolutionDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DynasysSolutionDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<DynasysSolutionDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<DynasysSolutionDbContext, Task<TResult>> func)
        {
            TResult result;
